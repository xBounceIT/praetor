...
const listVersionsMock = mock<(id: string) => Promise<OrderVersionRow[]>>(() =>
  Promise.resolve([]),
);
const getVersionMock = mock<(id: string, versionId: string) => Promise<OrderVersion>>(() =>
  Promise.reject(new Error('not configured')),
);
const restoreVersionMock = mock<(id: string, versionId: string) => Promise<ClientsOrder>>(() =>
  Promise.reject(new Error('not configured')),
);
...
describe('<OrderVersionsPanel />', () => {
  test('renders empty-state copy when no versions exist', async () => {
    listVersionsMock.mockImplementation(() => Promise.resolve([]));
    render(<OrderVersionsPanel {...baseProps} />);
    await waitFor(() =>
      expect(screen.getByText('clientsOrders.versionHistory.empty')).toBeInTheDocument(),
    );
  ...
  test('renders one row per version with reason badge for "Save" vs "Restored"', async () => {
    ...
    render(<OrderVersionsPanel {...baseProps} />);
    await waitFor(() =>
      expect(screen.getByText('clientsOrders.versionHistory.reasonRestore')).toBeInTheDocument(),
    );
    expect(screen.getByText('clientsOrders.versionHistory.reasonUpdate')).toBeInTheDocument();
  ...
  test('shows error when listVersions rejects', async () => {
    ...
    render(<OrderVersionsPanel {...baseProps} />);
    await waitFor(() =>
      expect(screen.getByText('clientsOrders.versionHistory.loadFailed')).toBeInTheDocument(),
    );
  });
  ...
  test('clicking a row fetches the full version and calls onPreview', async () => {
    ...
    await waitFor(() => expect(getVersionMock).toHaveBeenCalledWith('co-1', 'orv-2'));
  ...
  test('selected version reveals "Back to current" and "Restore" actions', async () => {
    ...
    render(<OrderVersionsPanel {...baseProps} selectedVersionId="orv-1" />);
    await waitFor(() =>
      expect(screen.getByText('clientsOrders.versionHistory.backToCurrent')).toBeInTheDocument(),
    );
    expect(screen.getByText('clientsOrders.versionHistory.restoreButton')).toBeInTheDocument();
  });
  ...
  test('"Back to current" button calls onClearPreview', async () => {
    ...
    fireEvent.click(screen.getByText('clientsOrders.versionHistory.backToCurrent'));
    expect(onClearPreview).toHaveBeenCalled();
  });

  test('Restore button is disabled when `disabled` prop is true', async () => {
    listVersionsMock.mockImplementation(() => Promise.resolve([VERSION_ROW_UPDATE]));
    render(<OrderVersionsPanel {...baseProps} selectedVersionId="orv-1" disabled />);
    await waitFor(() =>
      expect(screen.getByText('clientsOrders.versionHistory.restoreButton')).toBeInTheDocument(),
    );
    const restoreBtn = screen
      .getByText('clientsOrders.versionHistory.restoreButton')
  ...
  test('Restore flow: confirm → restoreVersion called → onRestored called → list reloaded', async () => {
    ...
    render(<OrderVersionsPanel {...baseProps} selectedVersionId="orv-1" onRestored={onRestored} />);
    await waitFor(() =>
      expect(screen.getByText('clientsOrders.versionHistory.restoreButton')).toBeInTheDocument(),
    );

    // Open confirm modal
    fireEvent.click(screen.getByText('clientsOrders.versionHistory.restoreButton'));
    ...
    await waitFor(() => expect(restoreVersionMock).toHaveBeenCalledWith('co-1', 'orv-1'));
