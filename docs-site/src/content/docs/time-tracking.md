...
Dalla lista attività della vista giornaliera puoi **Duplicare** una registrazione esistente: si apre un dialog in cui selezioni uno o più giorni di destinazione. Praetor crea **sempre** nuove entry con lo stesso cliente, progetto, attività, ore, note e luogo (`isPlaceholder` sempre `false`), accodandole a ciò che è già registrato quel giorno — anche quando lo stesso progetto/attività è già presente. Il giorno di origine non è selezionabile. Se alcuni giorni scelti hanno già quel progetto/attività, il dialog mostra un avviso informativo ma consente comunque di confermare. Le create della duplica non sono idempotenti: se una richiesta fallisce o va in timeout, controlla il calendario prima di duplicare di nuovo quei giorni, per non aggiungere una seconda riga fatturabile.

`POST /api/entries` inserisce sempre una nuova riga. Sono ammesse più entry per la stessa combinazione utente, data, progetto e attività. `PUT /api/entries/:id` risponde con `409` solo quando la versione di optimistic-lock è obsoleta.
...
Ogni registrazione esistente occupa una propria riga. La riga "Nuova voce" in alto serve esclusivamente a creare nuove registrazioni; sullo stesso giorno sono ammesse più righe con lo stesso progetto e attività.
