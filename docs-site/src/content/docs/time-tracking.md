...
Dalla lista attività della vista giornaliera puoi **Duplicare** una registrazione esistente: si apre un dialog in cui selezioni uno o più giorni di destinazione. Praetor crea nuove entry con lo stesso cliente, progetto, attività, ore, note e luogo (`isPlaceholder` sempre `false`), **accodandole** a ciò che è già registrato quel giorno. Il giorno di origine non è selezionabile. I giorni che hanno già **altri** progetti/attività sono ammessi. I giorni che hanno già lo **stesso** progetto/attività non possono ricevere una seconda copia (chiave unica): vengono saltati con un avviso. Altri errori restano riportati come esito parziale.

Praetor mantiene al massimo una riga per la stessa combinazione utente, data, progetto e attività. `POST /api/entries` risponde con `409` se esiste già una registrazione non-segnaposto per quella chiave (i segnaposto corrispondenti vengono promossi). `PUT /api/entries/:id` risponde con `409` se una modifica di data, progetto o attività colliderebbe con un'altra riga.
