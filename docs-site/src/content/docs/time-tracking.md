...
Dalla lista attività della vista giornaliera puoi **Duplicare** una registrazione esistente: si apre un dialog in cui selezioni uno o più giorni di destinazione. Praetor crea subito nuove entry con lo stesso cliente, progetto, attività, ore, note e luogo (`isPlaceholder` sempre `false`). Il giorno di origine non è selezionabile. I giorni che hanno già altre ore su progetti/attività diversi sono ammessi. Se un giorno scelto ha già un **segnaposto** per lo stesso progetto/attività (ad esempio da generazione ricorrente), Praetor promuove quel stub nella copia. Se esiste già una registrazione reale con la stessa combinazione utente/progetto/attività, `POST /api/entries` risponde con `409` e la duplicazione riporta un esito parziale.

Praetor non consente una seconda registrazione reale per lo stesso utente, data, progetto e attività: `POST /api/entries` risponde con `409` se la combinazione esiste già come non-segnaposto e `PUT /api/entries/:id` risponde con `409` se una modifica di data, progetto o attività produrrebbe la stessa combinazione. Creare su un segnaposto corrispondente lo promuove. Aggiorna la registrazione esistente invece di crearne una duplicata.
