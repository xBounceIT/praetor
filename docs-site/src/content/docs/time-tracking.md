...
Dalla lista attività della vista giornaliera puoi **Duplicare** una registrazione esistente: si apre un dialog in cui selezioni uno o più giorni di destinazione. Praetor crea subito nuove entry con lo stesso cliente, progetto, attività, ore, note e luogo (`isPlaceholder` sempre `false`). Il giorno di origine non è selezionabile. I giorni che hanno già altre ore su progetti/attività diversi sono ammessi. Se un giorno scelto ha già lo stesso progetto/attività — anche come segnaposto ricorrente — Praetor sovrascrive quella riga (`overwriteExisting`) e mostra un avviso nel dialog e nel toast. Altri errori restano riportati come esito parziale.

Praetor mantiene al massimo una riga per la stessa combinazione utente, data, progetto e attività. Un `POST /api/entries` normale senza `overwriteExisting` risponde con `409` se esiste già una registrazione non-segnaposto per quella chiave (i segnaposto vengono comunque promossi). Con `overwriteExisting: true` le righe corrispondenti vengono aggiornate. `PUT /api/entries/:id` risponde con `409` se una modifica di data, progetto o attività colliderebbe con un'altra riga.
