...
Dalla lista attività della vista giornaliera puoi **Duplicare** una registrazione esistente: si apre un dialog in cui selezioni uno o più giorni di destinazione. Praetor crea subito nuove entry con lo stesso cliente, progetto, attività, ore, note e luogo (`isPlaceholder` sempre `false`). Il giorno di origine non è selezionabile. Se per un giorno scelto esiste già la stessa combinazione utente/progetto/attività, `POST /api/entries` risponde con `409` e la duplicazione riporta un esito parziale.
