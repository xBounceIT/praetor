...
- **Importa da Excel** permette di scaricare `praetor-clients-import.xlsx`, compilare le celle evidenziate e importare fino a 500 clienti. Il modello include istruzioni, una riga di esempio non importata e tendine per i campi tipizzati. Le opzioni di settore, numero dipendenti, fatturato e numero sedi vengono lette dal CRM ogni volta che scarichi il modello, quindi includono subito le modifiche più recenti.
...
L'azione **Aggiungi Commessa** apre una finestra dedicata alla sola creazione: ordine cliente, cliente, nome, date, offerta facoltativa, tipo, stato, consuntivazione, ricavo facoltativo e una tabella di attività iniziali. Al salvataggio sei portato direttamente alla pagina di dettaglio della nuova commessa.

Cliccando una riga nell'archivio commesse si apre la **pagina di dettaglio della commessa**, che sostituisce la vecchia finestra di modifica ed è organizzata in due sezioni:

- La sezione superiore dispone i campi della commessa orizzontalmente (ordine cliente, cliente, nome, descrizione, date, offerta, tipo, stato, consuntivazione, ricavo, switch di disabilitazione) accanto alla tabella attività modificabile inline.
...
- **Data inizio commessa** e **Data fine commessa** — definiscono la finestra temporale prevista. Entrambe sono obbligatorie (alla creazione e a ogni salvataggio successivo dalla pagina di dettaglio) così che le commesse abbiano sempre una finestra di pianificazione; la data di fine non può precedere la data di inizio.
- **Ordine cliente** — collega la commessa a un ordine cliente confermato. Il campo è obbligatorio alla creazione e al salvataggio dalla pagina di dettaglio; quando scegli un ordine, il cliente della commessa viene impostato dall'ordine e resta bloccato.
- **Riferimento offerta** — collega la commessa a un'offerta accettata quando serve tracciarne l'origine commerciale. Il campo è facoltativo e può restare vuoto.
- **Tipo** — classifica la commessa come **Attivo** o **Passivo**. È un campo obbligatorio (con lo stesso indicatore `*` di Cliente e Nome Commessa): la commessa non può essere creata finché non scegli un valore, e il tipo selezionato è mostrato nell'archivio commesse e nella pagina di dettaglio. Le commesse già esistenti prima dell'introduzione del campo sono impostate su **Attivo** in modo predefinito, ma alla **prima modifica** dalla pagina di dettaglio devi confermare esplicitamente il tipo: il selettore parte vuoto e il salvataggio è bloccato finché non scegli un valore, così la scelta non resta quella predefinita per inerzia.
