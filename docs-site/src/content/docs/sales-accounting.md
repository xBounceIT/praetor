...
Lo **Stato** del preventivo fornitore segue lo stesso modello a sei stati del preventivo cliente. Quando il preventivo fornitore è **collegato** a un preventivo cliente (dal form del preventivo cliente), il suo stato è **sincronizzato** e di sola lettura: rispecchia lo stato del preventivo cliente collegato e non può essere modificato manualmente. Fa eccezione lo stato **Scaduto**, calcolato sempre dalla **Data Scadenza propria** del preventivo fornitore: un fornitore può quindi risultare Scaduto anche se il preventivo cliente collegato è ancora valido. La **Data Scadenza** resta modificabile in qualsiasi stato per rinnovare la validità; finché non è collegato a un preventivo cliente, il preventivo fornitore resta in **Bozza** con stato modificabile.
