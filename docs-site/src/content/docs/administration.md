...
Praetor supporta l'autenticazione a due fattori basata su TOTP (app authenticator come Google Authenticator, Authy o 1Password) per gli account con credenziali locali o LDAP. Ogni utente abilita la 2FA dalle proprie **Impostazioni → Sicurezza**. Per sicurezza, l'attivazione da una sessione già autenticata richiede prima di reinserire la password dell'account (così una sessione compromessa da sola non può registrare un secondo fattore); Praetor mostra poi un codice QR (e una chiave da inserire manualmente) da scansionare con l'app authenticator, chiede un codice a sei cifre per confermare e infine mostra una serie di **codici di backup** monouso da conservare al sicuro. I codici di backup vengono mostrati una sola volta; puoi rigenerarli in qualsiasi momento — invalidando i precedenti — inserendo un codice valido.
...
**Imporre la 2FA agli amministratori.** Nella sezione **Autenticazione** puoi attivare **Richiedi l'autenticazione a due fattori per gli amministratori**. Quando è attiva, un amministratore con credenziali locali o LDAP che non ha ancora configurato la 2FA viene indirizzato alla procedura di attivazione al successivo accesso e ottiene una sessione solo dopo averla completata. L'imposizione considera qualsiasi ruolo amministrativo assegnato all'utente, non solo il ruolo attivo. Attivando l'opzione, le sessioni già aperte e i token API (token di accesso personali e token MCP) degli amministratori senza 2FA vengono revocati nella stessa transazione che salva il criterio, così non possono conservare i privilegi di amministratore (nemmeno tramite il cambio ruolo o le API) fino alla loro scadenza. Le credenziali vengono revocate allo stesso modo ogni volta che un utente senza 2FA diventa amministratore mentre l'obbligo è attivo — per promozione a un ruolo amministrativo o per cambio del metodo di autenticazione verso uno compatibile con la 2FA (locale o LDAP) — così non può agire da amministratore senza prima configurare la 2FA. Un amministratore soggetto all'obbligo non può disattivare la propria 2FA dalle impostazioni: solo un reset da parte di un altro amministratore può rimuoverla. Poiché l'opzione viene salvata tramite le impostazioni generali, il toggle compare solo agli amministratori che hanno il permesso di modifica delle impostazioni generali.
