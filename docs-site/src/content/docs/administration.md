...
Nella scheda **Capacità AI**, abilita AI reporting e scegli **Gemini**, **OpenRouter** oppure **Anthropic**. Ogni provider conserva una chiave API e un ID modello separati, così il passaggio da un provider all'altro non sovrascrive la configurazione precedente. Per Anthropic, crea la chiave nella Anthropic Console, inserisci l'ID esatto di un modello Claude disponibile per l'account e usa **Verifica** prima di salvare. Le richieste AI reporting vengono inviate dal server direttamente al provider selezionato.
