...
I contatori sono separati per modulo e anno e ripartono da `0001` all'inizio di ogni anno. Le fatture usano l'anno della **Data emissione**, mentre preventivi, offerte e ordini usano la data corrente del server. Cambiare un template influenza solo i documenti creati in futuro: i codici già assegnati non vengono rinominati. I campi codice nei form restano modificabili come override manuale; lasciandoli vuoti Praetor assegna il codice automatico e salta eventuali collisioni con codici storici o inseriti manualmente.
