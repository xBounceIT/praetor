...
La sincronizzazione LDAP manuale richiede una configurazione LDAP abilitata. Il pulsante **Sincronizza utenti ora** nelle impostazioni LDAP esegue subito la sincronizzazione usando la configurazione già salvata e rispetta l'interruttore **Provisioning massivo durante la sincronizzazione**: crea gli utenti mancanti solo quando quell'opzione è attiva, altrimenti aggiorna soltanto gli utenti LDAP già presenti. Se LDAP è disabilitato o non configurato, la richiesta viene respinta e non viene registrata come sincronizzazione riuscita; se la directory non è raggiungibile, Praetor segnala l'errore invece di riportare un successo.
...
Per un utente LDAP già presente, Praetor accetta le credenziali solo quando lo username canonico restituito dalla directory (`uid` o `sAMAccountName`) corrisponde allo username memorizzato nell'account Praetor, senza distinzione tra maiuscole e minuscole. Un alias come email o UPN continua a funzionare quando viene prima risolto allo username canonico; se invece l'alias coincide con lo username di un altro account Praetor, l'accesso viene rifiutato. La stessa verifica protegge la conferma della password LDAP richiesta per configurare la 2FA.
...
**Il criterio MFA.** Il criterio 2FA si trova in una scheda **MFA** dedicata nelle impostazioni di **Autenticazione** ed espone cinque controlli. Poiché queste impostazioni vengono salvate tramite le impostazioni generali, la scheda compare solo agli amministratori che hanno il permesso di modifica delle impostazioni generali.
...
- **Utenti esenti** — selezione multipla dei singoli utenti che non sono mai obbligati a usare la 2FA. Questa esenzione prevale anch'essa sull'imposizione per ruolo e serve per eccezioni puntuali senza creare ruoli dedicati.

Un utente è obbligato a usare la 2FA solo quando **Abilita 2FA** e **Imponi 2FA** sono entrambi attivi, l'account usa credenziali locali o LDAP, l'utente non è tra gli **Utenti esenti**, nessuno dei ruoli dell'utente è tra i **Ruoli esenti** e, inoltre, **Imponi per i ruoli** è vuoto oppure uno dei ruoli dell'utente vi è elencato.
...
Nella scheda **Capacità AI**, abilita AI reporting e scegli **Gemini**, **OpenRouter**, **Anthropic**, **OpenAI** oppure **Locale (OpenAI-compatible)**. Ogni provider conserva credenziali e ID modello separati, così il passaggio da un provider all'altro non sovrascrive la configurazione precedente. Per Anthropic, crea la chiave nella Anthropic Console e inserisci l'ID esatto di un modello Claude disponibile per l'account. Per OpenAI, Praetor usa la Responses API e non richiede un ID assistente o una conversazione configurata sulla piattaforma. Usa **Verifica** prima di salvare. Le richieste AI reporting vengono inviate dal server direttamente al provider selezionato.

Il provider locale usa l'API OpenAI Chat Completions ed è compatibile con server come Ollama, vLLM e LocalAI. Configura una base URL che includa il prefisso API, per esempio `http://inference:11434/v1`, l'ID esatto del modello e, solo se richiesto dal server, un token Bearer. L'endpoint deve essere raggiungibile dal container backend di Praetor: `localhost` indica quel container e non il computer dell'utente o l'host Docker. Usa quindi il nome DNS del servizio/container oppure un indirizzo di rete raggiungibile dal backend. Praetor accetta HTTP e HTTPS per consentire l'uso in LAN, non segue redirect e rifiuta destinazioni link-local o metadata.

Nella scheda **Codici documento**, gli amministratori con permesso di modifica delle impostazioni generali definiscono i template globali usati per generare i codici di preventivi clienti, offerte clienti, preventivi fornitori, ordini clienti, ordini fornitori, fatture clienti e fatture fornitori. Ogni modulo ha un prefisso, un template e il numero di cifre della sequenza (da 1 a 9, predefinito 4). I placeholder disponibili sono `{PREFIX}`, `{YY}`, `{YYYY}` e `{SEQ}`; `{SEQ}` e un placeholder anno (`{YY}` o `{YYYY}`) sono obbligatori. Il testo fisso del template può contenere solo lettere, numeri, underscore e trattini, così i codici restano utilizzabili nelle URL dei documenti. Praetor mostra un'anteprima live e respinge template vuoti, placeholder sconosciuti, prefissi non validi o codici renderizzati più lunghi del limite di 100 caratteri.

I contatori sono separati per modulo e anno e ripartono da `0001` all'inizio di ogni anno: il progressivo di un nuovo anno nasce indipendente da quello dell'anno precedente. Le fatture usano l'anno della **Data emissione**, mentre preventivi, offerte e ordini usano la data corrente del server. Cambiare un template influenza solo i documenti creati in futuro: i codici già assegnati non vengono rinominati.

Quando un documento nasce da un documento sorgente, Praetor riusa anno e contatore del codice sorgente nel modulo di destinazione: un preventivo `PREV_26_0045` può generare un'offerta `OFF_26_0045` anche se `OFF_26_0044` non esiste. Questi buchi sono previsti. La stessa regola vale per codici sorgente o override manuali conformi alla forma `prefisso_anno_contatore_...` con separatore `_`; un override manuale parseabile riserva il contatore del proprio modulo almeno al valore successivo. Se il codice derivato è già occupato, Praetor segnala una collisione invece di avanzare automaticamente a un altro progressivo. Per le fatture clienti, se esiste già una fattura collegata allo stesso ordine, le fatture successive usano invece il progressivo sequenziale del modulo.

Nella scheda **Personalizzazione**, gli amministratori personalizzano il nome e il logo dell'azienda. La barra laterale mantiene sempre la scritta "PRAETOR" come marchio prodotto e mostra il nome azienda nella riga secondaria sotto il marchio, al posto dell'etichetta ruolo/area di lavoro; lo stesso nome identifica automaticamente il cliente delle commesse interne e viene sincronizzato quando cambia. L'etichetta di versione in basso resta "Praetor v...". Il logo caricato sostituisce sia l'icona nella barra laterale sia il logo della schermata di accesso. I logo accettano PNG, JPEG, WEBP o SVG fino a 2 MB; lasciando vuoto un campo si ripristina il valore predefinito di Praetor. La personalizzazione è leggibile pubblicamente affinché la schermata di accesso possa mostrarla prima dell'accesso, ma solo gli amministratori con il permesso di aggiornamento delle impostazioni generali possono modificarla.
