...
Nella scheda **Capacità AI**, abilita AI reporting e scegli **Gemini**, **OpenRouter**, **Anthropic**, **OpenAI** oppure **Ollama**. Ogni provider conserva credenziali e ID modello separati, così il passaggio da un provider all'altro non sovrascrive la configurazione precedente. Per Anthropic, crea la chiave nella Anthropic Console e inserisci l'ID esatto di un modello Claude disponibile per l'account. Per OpenAI, Praetor usa la Responses API e non richiede un ID assistente o una conversazione configurata sulla piattaforma. Per Ollama, indica l'URL raggiungibile dal container backend, il modello locale e, solo se richiesto dal reverse proxy, un token Bearer. Usa **Verifica** prima di salvare. Le richieste AI reporting vengono inviate dal server direttamente al provider o endpoint selezionato.
