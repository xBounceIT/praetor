...
Nella scheda **Capacità AI**, Gemini e OpenRouter richiedono una chiave API e l'identificativo esatto del modello. Se scegli **Ollama**, configura invece l'URL base del server Ollama e un modello già installato; il token Bearer è facoltativo e serve soltanto quando Ollama è pubblicato dietro un reverse proxy autenticato. L'URL viene risolto dal backend di Praetor, non dal browser: in un'installazione Docker `http://localhost:11434` punta al container backend e va sostituito con il nome servizio o l'host Ollama raggiungibile da quel container. Il pulsante **Verifica** controlla i modelli disponibili sull'istanza senza scaricarne di nuovi.
