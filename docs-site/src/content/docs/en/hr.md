...
Each competence center card separates managers from the team, shows the total member count, and keeps available actions visible. Managers and members use the same compact avatar group; hover an initial to read the full name. When people exceed the space available, a `+N` badge sums up the rest and, on hover, lists the complete group so you can see it without opening the card. Use **Manage Managers** below the manager column to change assignments in the dedicated dialog; the competence-center edit form now updates only the name and description. Managers use a stable name order that remains consistent between the card and the management dialog.
