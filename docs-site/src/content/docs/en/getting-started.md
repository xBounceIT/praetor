...
Active filters remain associated with each table in the browser: if you move to another section and come back, Praetor restores the selection automatically. When the table is opened normally, **Clear filters** removes both the visible filters and the values remembered for that table. Filters supplied by a direct link always take precedence but remain temporary: clearing them from the view does not replace the selection remembered by the browser.

Use the handle in a column header to drag that column into position; with the keyboard, use the left and right arrow keys on the same handle. The **Columns** menu also lets you show or hide available columns. After a manual order or visibility change, the **Save the custom view?** suggestion to the left of **Export** opens the new custom-view dialog prefilled with the current layout. You can also change the order in the column list while adding or editing a view, either by dragging the handle or using the up and down arrow keys. **Reset columns** restores both the default visibility and order. Unsaved manual changes are temporary, while a named view preserves and shares column visibility and order together with data sorting and filters.
