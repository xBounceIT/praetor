...
The `timesheets.expired_projects.create` permission allows entries dated after a project's end date. Historical entries within the project period do not require it. The built-in **Manager** and **Top Manager** roles receive it by default; grant it to other roles only when they need logging beyond the planned project period.
...
For Keycloak OIDC, set **Issuer URL** to the realm's public HTTPS URL (for example `https://iam.example.com/realms/company`), reachable from both the browser and backend. The issuer and discovered endpoints must meet the HTTPS/public-network policy above; a private or internal Docker address is rejected. Set `FRONTEND_URL` to the application's public HTTPS URL and, if the callback uses another origin, set `SSO_CALLBACK_BASE_URL` to the HTTPS origin receiving `/api/auth/sso/oidc/<slug>/callback`. Neither URL may contain credentials. Register that callback as an allowed redirect URI for the Keycloak client. If IAM login cannot start, the login page shows a translated message (`server_misconfigured` for the server's public URL, `provider_misconfigured` for provider discovery); if `FRONTEND_URL` is malformed or unsafe and the server cannot return to the login page, it responds with 503 instead. The technical detail appears in the **backend** logs under `SSO login start failed`.
...
For an existing LDAP user, Praetor accepts credentials only when the canonical username returned by the directory (`uid` or `sAMAccountName`) matches the username stored on the Praetor account, case-insensitively. An alias such as an email address or UPN still works when it is first resolved to the canonical username; if the alias instead collides with another Praetor account's username, sign-in is rejected. The same check protects the LDAP password confirmation required when setting up 2FA.
...
Listing or managing personal access tokens and MCP tokens from **Settings → Security** requires an interactive browser session. API tokens cannot inspect, create, rotate, or revoke these credentials.
...
In the **AI Capabilities** tab, enable AI reporting and choose **Gemini**, **OpenRouter**, **Anthropic**, **OpenAI**, or **Local (OpenAI-compatible)**. Each provider keeps separate credentials and a model ID, so switching providers does not overwrite the previous configuration. Provider keys are encrypted at rest and are write-only: after saving, the field shows a mask and neither administrators nor API clients can retrieve the original value. Leave the mask unchanged to retain the stored key, or replace it to rotate the credential. For Anthropic, create the key in Anthropic Console and enter the exact ID of a Claude model available to the account. For OpenAI, Praetor uses the Responses API and does not require an assistant ID or a preconfigured platform conversation. Use **Check** before saving. AI reporting requests are sent by the server directly to the selected provider.

The local provider uses the OpenAI Chat Completions API and works with servers such as Ollama, vLLM, and LocalAI. Configure a base URL that includes the API prefix, for example `http://inference:11434/v1`, the exact model ID, and a Bearer token only when the server requires one. The endpoint must be reachable from Praetor's backend container: `localhost` means that container, not the user's computer or the Docker host. Use the service/container DNS name or a network address reachable by the backend. Praetor accepts HTTP and HTTPS for LAN deployments, does not follow redirects, and rejects link-local or metadata destinations.
...
In the same tab, **Revision codes** configures the prefix, template, and padding used for sent quote and offer snapshots. The default is prefix `REV`, template `{PREFIX}{SEQ}`, and padding `1`, so the first revision is `REV1`. `{SEQ}` is required and the counter is separate for every individual document rather than per module or year. Changes apply only to future revisions: existing revision codes remain immutable, and assigned numbers stay reserved after a restore.
...
In **Branding**, administrators customize the company name and logo. The sidebar always keeps the "PRAETOR" product wordmark and shows the company name in the secondary line under it, replacing the role/workspace label; that same name automatically identifies the client for Internal jobs and stays synchronized when Branding changes. The bottom version label remains "Praetor v...". The uploaded logo replaces both the sidebar icon and the login-screen logo. Logos accept PNG, JPEG, WEBP, or SVG up to 2 MB; leaving a field empty restores the bundled Praetor default. Branding is readable publicly so the login screen can show it before sign-in, but only administrators with the general-settings update permission can change it.
