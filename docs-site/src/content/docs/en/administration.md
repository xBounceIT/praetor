...
In **Tracking Preferences**, administrators also configure the metadata used by the RIL statement: company name, default start time, and lunch-break minutes. These values are printed in the RIL Excel workbook header; they do not change existing time entries.
