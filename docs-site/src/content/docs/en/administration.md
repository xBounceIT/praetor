...
**The MFA policy.** The 2FA policy lives on a dedicated **MFA** tab in the **Authentication** settings and exposes four controls. Because these settings are saved through the general settings, the tab is shown only to administrators who hold the general-settings update permission.
...
A user is required to use 2FA only when **Enable 2FA** and **Enforce 2FA** are both on, the account uses local or LDAP credentials, none of the user's roles is in **Exempt roles**, and either **Enforce for roles** is empty or one of the user's roles is listed there.
...
Counters are separate per module and year and restart from `0001` at the beginning of each year. Invoices use the **Issue Date** year, while quotes, offers, and orders use the server's current date. Changing a template affects only documents created in the future: existing codes are not renamed. Code fields in create forms remain editable for manual overrides; leaving them blank lets Praetor assign the automatic code and skip any collision with historical or manually entered codes.

In **Branding**, administrators customize the company name and logo. The company name replaces the "PRAETOR" wordmark in the sidebar header, while the bottom version label remains "Praetor v..."; the uploaded logo replaces both the sidebar icon and the login-screen logo. Logos accept PNG, JPEG, WEBP, or SVG up to 2 MB; leaving a field empty restores the bundled Praetor default. Branding is readable publicly so the login screen can show it before sign-in, but only administrators with the general-settings update permission can change it.
