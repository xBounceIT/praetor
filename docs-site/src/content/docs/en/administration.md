...
The **Users** page stays focused on application access: username, role, permissions, authentication method, and account status. HR data such as company phone, work email, role, department derived from competence centers, responsible user, contract, location, and personal contacts is managed in the **HR** module, not in Administration.
