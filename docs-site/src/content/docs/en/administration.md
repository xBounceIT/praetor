...
In **AI Capabilities**, Gemini and OpenRouter require an API key and the exact model identifier. When you select **Ollama**, configure the Ollama server base URL and an already-installed model instead; the Bearer token is optional and is only needed when Ollama is exposed through an authenticated reverse proxy. The URL is resolved by the Praetor backend rather than the browser: in Docker, `http://localhost:11434` points to the backend container and must be replaced with the Ollama service name or a host reachable from that container. **Check** lists the models available on the instance and never downloads a model.
