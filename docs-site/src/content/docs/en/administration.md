...
In the **AI Capabilities** tab, enable AI reporting and choose **Gemini**, **OpenRouter**, **Anthropic**, **OpenAI**, or **Ollama**. Each provider keeps separate credentials and a model ID, so switching providers does not overwrite the previous configuration. For Anthropic, create the key in Anthropic Console and enter the exact ID of a Claude model available to the account. For OpenAI, Praetor uses the Responses API and does not require an assistant ID or a preconfigured platform conversation. For Ollama, provide the URL reachable from the backend container, the local model, and a Bearer token only when required by a reverse proxy. Use **Check** before saving. AI reporting requests are sent by the server directly to the selected provider or endpoint.
