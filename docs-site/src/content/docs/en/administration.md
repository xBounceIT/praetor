...
**Enforcing 2FA for administrators.** In the **Authentication** section you can turn on **Require two-factor authentication for administrators**. When enabled, an administrator with local or LDAP credentials who has not yet set up 2FA is routed into enrollment on their next sign-in and only receives a session once it is completed. Enforcement considers any admin role assigned to the user, not just their active role. Turning the option on does not log anyone out: existing browser sessions of admins without 2FA stay active (and the policy takes hold the next time they sign in), while attempting to switch into an admin role without a second factor is blocked. It does, however, revoke the API tokens (personal access tokens and MCP tokens) of those admins — in the same transaction that saves the policy — because such tokens never pass through the login enrollment step and would otherwise keep administrator API access with no second factor. Credentials are revoked the same way whenever a user without 2FA becomes an administrator while the mandate is on — by being promoted into an admin role or by having their authentication method changed to a TOTP-applicable one (local or LDAP) — so they cannot act as an administrator until they set up 2FA. An administrator subject to the mandate cannot disable their own 2FA from settings; only an admin reset can remove it. Because the option is saved through the general settings, the toggle is shown only to administrators who hold the general-settings update permission.
