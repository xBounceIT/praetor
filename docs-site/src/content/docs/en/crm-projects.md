...
In the create or edit product form, the **Manage** button above the **Type**, **Category**, and **Subcategory** fields opens the corresponding list of values. From there you can add or rename a value and delete it when existing links do not protect it. Type and category names must be unique case-insensitively.
...
Use estimated monthly effort to plan recurring load and task duration as a generic multiplier. Total effort is calculated automatically as monthly effort × duration and is used to track progress against the overall expected hours. Task total revenue is calculated the same way: revenue × duration. Monthly effort and revenue are optional for initial tasks on **Internal** jobs and remain required for commercial jobs. Logged-hour totals apply task visibility and timesheet visibility independently: they show your own hours, include managed users for holders of the Timesheets permission, and include every user only with **All timesheets - View** (`timesheets.tracker_all.view`).
...
- The top section lays out the job fields horizontally (type, client, name, description, dates, status, billing, revenue, disabled toggle, plus order and offer for commercial jobs) next to the inline-editable job tasks table.
...
- **Job start date** and **Job end date** — define the planned window. For **Active** and **Passive** jobs the start date is required; the end date is required unless status is **Perpetuo**, in which case it stays optional. **Internal** jobs may omit either or both so company activity can remain open-ended. When both are set, the end date must not precede the start date. Switching from Internal to a commercial type, or leaving **Perpetuo** on a commercial job, requires both dates in the same save when they are missing.
- **Client order** — links an Active or Passive job to a confirmed customer order. It is required for those types at creation and save; choosing one sets and locks the job client. Internal jobs cannot have an order.
- **Offer reference** — optionally links an Active or Passive job to an accepted offer. Internal jobs cannot have an offer.
- **Type** — classifies the job as **Active**, **Passive**, or **Internal**. It is required and shown in the archive and detail page. Internal jobs track company activity without an order, offer, or mandatory planning window; Praetor manages their client using the company name from Branding, while status, billing, revenue, tasks, timesheets, and normal permissions remain unchanged. Converting a linked job to Internal opens a confirmation that clears the local order and offer so all three changes are saved together; cancelling changes nothing. Switching back to Active or Passive restores the commercial fields and blocks saving until a confirmed order matching the client and both required dates are provided. Jobs that predate the type field remain Active and require explicit confirmation on their first edit.
- **Status** — tracks the job lifecycle and is visible as a column in the jobs archive. The values are **Da fare** (planned job, time entries allowed), **In corso** (active job, time entries allowed), **In pausa** (paused job, time entries blocked), **Terminato** (closed job, time entries blocked), and **Perpetuo** (permanent job, time entries allowed; end date optional and never treated as expired for timesheets even when in the past). The selector, information summary, and badges use consistent icons: a Stop square for **Da fare**, Play for **In corso**, Pause for **In pausa**, a checkmark for **Terminato**, and an infinity symbol for **Perpetuo**. The form shows an information icon next to the **Status** label with a short hover/focus summary. New jobs start as **Da fare**; existing jobs are initialized as **In corso**. **In pausa** and **Terminato** do not disable the job record: it remains visible in management for history and reopening by changing status.
...
The **Job rules** section on the detail page lets you create automatic controls from operational data that can evolve: name and description, enabled state, dates, revenue, billing type and frequency, lifecycle status, active/passive/internal classification, and type confirmation. Technical IDs and client, order, or offer links are not offered as conditions. Calculated metrics include logged cost and hours, budget percentage, days since start or until deadline, task counts, and planned effort. Hovering over a field in the selector shows a description of the value and how it is calculated. Text fields support equality, containment, prefix, and suffix; nullable text, date, and number fields also support **empty/not-empty** checks, while enums and booleans expose operators appropriate to their type. A condition can compare a field with a value or another compatible field, and multiple conditions can be combined with **AND** or **OR**.

Each rule can use **Continuous monitoring** or a **Periodic check**. Continuous monitoring evaluates current values and runs only when the condition changes from false to true. A periodic rule evaluates the latest completed day, week, month, quarter, or year once using the application time zone automatically, so it can run again on every cycle without duplicating a period. **Every month** runs on the first day; choosing **Custom** opens the same dialog used by recurring tasks, where you can select the first, second, third, fourth, or last weekday of the month, such as the first Monday or last Friday. The check always analyzes the previous calendar month. A new rule starts from the latest period already due; after service downtime, due but unevaluated periods are caught up in order instead of being skipped. Period metrics include hours, cost, entry count, active users, and used tasks. They can be filtered by one or more users and tasks. For example, to check every month whether user X logged time on task Y, choose **Every month** or a **Custom** pattern, select X and Y in the filters, and set **Hours in period = 0**. An empty filter includes every job user or task. Historical entries whose task has been deleted remain included in unfiltered checks, but they are not reassigned to a current task with the same name and are therefore excluded from task-filtered checks. If a user is removed from the job without historical time entries, or a selected user or task is deleted, the check is suspended to prevent false alerts: the filter is shown as **no longer available** and can be removed to resume checks.

When a rule is met, it can run one or more actions: notify selected assigned users or assigned users holding selected roles, and send a JSON event to an administrator-enabled webhook. Available roles must be held by at least one enabled assigned user, and role notifications never expand beyond job assignments. Webhook targets require **View webhooks** (`administration.webhooks.view`); existing hidden webhook actions are preserved when other details change. The payload includes the configuration, completed period, and available metrics. Cost conditions and their webhook values require **Cost reports**; without that permission, the rule is not exposed. The section uses **Project Rules** (`projects.rules`), granted to Managers and Top Managers by default. Re-enabling a rule or changing its conditions, mode, or schedule resets evaluation state for the next check.

### Assigning users

The **Assign Users** / **Manage Members** command in the job detail header manages who is assigned to the **job**. To assign members to an **activity**, use the action in the **Actions** column of the **Job activities** table (the same action is also available from the Activities view). Access to these dialogs is governed by the **Project Assignments** permission: the **View** action lets a role open the assignments of any job or activity regardless of its own membership, while **Update** lets it edit them. Task-edit permissions do not grant assignment-management access. Managers and Top Managers hold both assignment permissions by default, so they can manage assignments even when they are not members of the job or activity. For tasks, the server accepts only enabled users who are visible to the editor and eligible for assignment; disabled users, Top Managers, and admin-only users are rejected even in direct API requests.
