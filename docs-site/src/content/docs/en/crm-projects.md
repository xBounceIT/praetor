...
- **Type** — classifies the job as **Active**, **Passive**, or **Internal**. It is required and shown in the archive and detail page. Internal jobs track company activity without an order or offer; Praetor manages their client using the company name from Branding, while dates, status, billing, revenue, tasks, timesheets, and normal permissions remain unchanged. Converting a linked job to Internal opens a confirmation that clears the local order and offer so all three changes are saved together; cancelling changes nothing. Switching back to Active or Passive restores the commercial fields and blocks saving until a confirmed order matching the client is selected. Jobs that predate the type field remain Active and require explicit confirmation on their first edit.
