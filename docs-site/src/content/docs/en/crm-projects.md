...
Next to the analytics section heading are two buttons, **Edit** and **Views**, that work like the custom column views on data tables. **Edit** opens the layout editor: resize each visualization (half or full width), hide it, or move it to another slot, then either keep the arrangement for this project or save it as a reusable view. Editing a project's dashboard creates a **per-project layout** that affects only that project. The **Views** menu lets you apply one of your saved views, choose **Use global default** (drop this project's custom layout so it follows the shared default again), or choose **Set as global default** (make the current arrangement the baseline for every project that doesn't have its own layout). Saved views and the global default are shared across projects; per-project layouts are remembered per project. Everything is stored locally in your browser.
