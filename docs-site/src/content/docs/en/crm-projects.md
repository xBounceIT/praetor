...
- **Job start date** and **Job end date** — define the planned window. Both are required for **Active** and **Passive** jobs, while **Internal** jobs may omit either or both so company activity can remain open-ended. When both are set, the end date must not precede the start date. Switching from Internal to a commercial type requires both dates in the same save.
...
- **Status** — tracks the job lifecycle and is visible as a column in the jobs archive. The values are **Da fare** (planned job, time entries allowed), **In corso** (active job, time entries allowed), **In pausa** (paused job, time entries blocked), **Terminato** (closed job, time entries blocked), and **Perpetuo** (permanent job, time entries allowed). The selector, information summary, and badges use consistent icons: a Stop square for **Da fare**, Play for **In corso**, Pause for **In pausa**, a checkmark for **Terminato**, and an infinity symbol for **Perpetuo**. The form shows an information icon next to the **Status** label with a short hover/focus summary. New jobs start as **Da fare**; existing jobs are initialized as **In corso**. **In pausa** and **Terminato** do not disable the job record: it remains visible in management for history and reopening by changing status.
