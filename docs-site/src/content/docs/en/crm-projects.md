...
The **Job rules** section on the detail page lets you create automatic controls for a job. A rule compares one or more job fields (revenue, logged hours, days until deadline, billing, or status; cost fields require the **Cost reports** permission) against thresholds, values, or compatible fields and can combine conditions with **AND** or **OR**. Rules whose conditions reference cost fields are omitted from both the API response and the rule list when the user lacks **Cost reports**, including when a cost field is used as the comparison target. The status field uses the same lifecycle values as the job: **Da fare**, **In corso**, **In pausa**, **Terminato**, and **Perpetuo**. When the rule becomes true, it can run one or more actions: send notifications to selected assigned users or assigned users who hold selected roles, and send a JSON event to one of the webhook targets enabled by administrators. Only roles held by at least one enabled user assigned to the job are available; role notifications never expand beyond the job's assignments. Webhook targets are visible and selectable only with the **View webhooks** permission (`administration.webhooks.view`); without it, only notification actions can be configured. Existing hidden webhook actions are preserved when other rule details or notification recipients are edited, and the rule can be disabled without exposing its target, but it cannot be re-enabled without **View webhooks**. In the rule list, a hidden target appears only as a protected webhook action, without its name or identifier. The webhook payload includes the job, rule, and available metrics; cost metrics are included only when the rule condition uses cost fields. The section is shown and edited through the **Project Rules** permission (`projects.rules`), granted to Managers and Top Managers by default. Actions fire only on the transition from not met to met, so they are not duplicated while the rule remains true. Re-enabling a rule or changing its condition prepares it to run again on the next scheduled check.
