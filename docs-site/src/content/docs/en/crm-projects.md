...
Each rule can use **Continuous monitoring** or a **Periodic check**. Continuous monitoring evaluates current values and runs only when the condition changes from false to true. A periodic rule evaluates the latest completed day, week, month, quarter, or year once using the application time zone automatically, so it can run again on every cycle without duplicating a period. For monthly checks, you can choose the first or any day from 2 through 31: the check runs on that day and analyzes the previous calendar month; in shorter months, unavailable dates are moved to the month's final day. A new rule starts from the latest period already due; after service downtime, due but unevaluated periods are caught up in order instead of being skipped. Period metrics include hours, cost, entry count, active users, and used tasks. They can be filtered by one or more users and tasks. For example, to check every month whether user X logged time on task Y, choose **Every month**, select the desired day, select X and Y in the filters, and set **Hours in period = 0**. An empty filter includes every job user or task. If a user is removed from the job without historical time entries, or a selected user or task is deleted, the check is suspended to prevent false alerts: the filter is shown as **no longer available** and can be removed to resume checks.
