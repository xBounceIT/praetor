...
From the daily activity list you can **Duplicate** an existing entry: a dialog lets you pick one or more target days. Praetor immediately creates new entries with the same client, project, task, hours, notes, and location (`isPlaceholder` always `false`). The source day cannot be selected. Days that already have other projects/tasks are allowed. If a chosen day already has the same project/task — including a recurring placeholder — Praetor overwrites that row (`overwriteExisting`) and shows a warning in the dialog and toast. Other failures still report a partial result.

Praetor keeps at most one row for the same user, date, project, and task. Normal `POST /api/entries` without `overwriteExisting` returns `409` when a non-placeholder entry already exists for that key (placeholders still promote). With `overwriteExisting: true`, matching rows are updated instead. `PUT /api/entries/:id` returns `409` when a date, project, or task change would collide with another row.
