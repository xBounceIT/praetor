...
From the daily activity list you can **Duplicate** an existing entry: a dialog lets you pick one or more target days. Praetor immediately creates new entries with the same client, project, task, hours, notes, and location (`isPlaceholder` always `false`). The source day cannot be selected. Days that already have other projects/tasks are allowed. If a chosen day already has a **placeholder** for the same project/task (for example from recurring generation), Praetor promotes that stub into the copied entry. If a real entry with the same user/project/task already exists, `POST /api/entries` returns `409` and the duplication reports a partial result.

Praetor does not allow a second real entry for the same user, date, project, and task: `POST /api/entries` returns `409` when that combination already exists as a non-placeholder, and `PUT /api/entries/:id` returns `409` when a date, project, or task change would produce it. Creating onto a matching placeholder promotes that stub instead. Update the existing entry instead of creating a duplicate.
