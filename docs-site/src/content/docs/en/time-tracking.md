...
From the daily activity list you can **Duplicate** an existing entry: a dialog lets you pick one or more target days. Praetor creates new entries with the same client, project, task, hours, notes, and location (`isPlaceholder` always `false`) alongside whatever is already logged that day. The source day cannot be selected. Days that already have **other** projects/tasks are fine. Days that already have the **same** project/task cannot receive a second copy (unique key): those days are skipped with a warning. Other failures still report a partial result.

Praetor keeps at most one row for the same user, date, project, and task. `POST /api/entries` returns `409` when a non-placeholder entry already exists for that key (matching placeholders are promoted into a real entry). `PUT /api/entries/:id` returns `409` when a date, project, or task change would collide with another row.
