...
The client quote **Status** follows the **Draft → Sent → Offer → Accepted / Denied** pipeline. The **Expired** status is derived automatically from the **Expiration Date** (once it has passed) and takes precedence over the displayed status; an **Accepted** or **Denied** quote is never shown as Expired. A quote can return to **Draft** only from **Sent** or **Offer**: from Accepted, Denied, or Expired the transition is not allowed. You leave Expired only by extending the **Expiration Date** (always editable), not by changing the status manually.

From the client quote form you can link a single **Supplier Quote** (a 1-to-1 relationship) through the **Linked Supplier Quote** selector. While the linked supplier quote is **Expired**, the client quote shows an indicator and cannot progress to **Sent**, **Offer**, or **Accepted** until you extend the supplier quote's validity.
...
The supplier quote **Status** is fully derived and can never be changed manually. An unlinked supplier quote is always a **Draft** (and that is when it appears among the options in the client quote form); once linked to a client quote, the status follows that document: **Sent** when the client quote is sent, **Offer** once a client offer is created from the quote, **Accepted**/**Denied** when the quote or its offer reaches a terminal state. **Expired** appears when the supplier quote's **own Expiration Date** passes, or when the linked quote/offer expires. The **Expiration Date** stays editable in any status to renew validity.
...
Item quantities and costs stay in sync with the client documents that use them, in both directions: editing the quantity or cost of a supplier-sourced line inside a client quote or offer retroactively updates the supplier quote item (the list price is recomputed keeping the **Discount to Us**). Only a genuine edit is pushed — re-saving a document that still holds older values (for example after changing just the notes) never overwrites changes made directly on the supplier quote, and a freshly linked line always starts from the supplier quote's current values. Pushing the change also requires the supplier-quote update permission. Conversely, when a supplier quote is edited directly, the client quote/offer lines that use it show an **Old info — update?** button that pulls the latest values back into the line. Lines sourced from a supplier quote that already generated an order — or whose status is **Accepted**, **Denied**, or **Expired** — stay locked, and a supplier quote whose items are used by client quotes, offers, or orders can no longer be deleted. When a client order is created and a line-sourced supplier quote is not in **Accepted** status (only the supplier quote linked to the client document follows its status), no supplier order is generated for it and the response reports a warning.
