...
Rows that automatically generated a **supplier order** (identifiable from the *Supplier order* column) stay locked even in draft: they cannot be removed, nor can their product or quantity change, so the linked procurement order never falls out of sync. You can still update their sale price, add other rows, and edit the header fields. When a row is linked to a supplier order, the shortcut icon in the *Supplier order* column opens it — already filtered — in a new tab, just like the product shortcut, so you can review it without leaving the order you are editing.
