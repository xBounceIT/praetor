...
Customer offers follow the same expiration model as quotes: every offer has a mandatory **Expiration Date**, and once it passes a **Draft** or **Sent** offer shows the derived **Expired** status (an **Accepted** or **Denied** offer is never shown as Expired). An expired offer is read-only and cannot change status — the only way out is extending the **Expiration Date**, which stays editable from the offer form on sent and expired offers.
...
The supplier quote **Status** follows the same six-state model as the client quote. When the supplier quote is **linked** to a client quote (from the client quote form), its status is **synced** and read-only: it mirrors the linked client quote's status and cannot be changed manually. The **Expired** status is the exception — it is always computed from the supplier quote's **own Expiration Date**, so a supplier quote can be Expired even while its linked client quote is still valid. The **Expiration Date** stays editable in any status to renew validity; while it is not linked to a client quote, the supplier quote stays in **Draft** with an editable status.
