...
const StatusBadge: React.FC<StatusBadgeProps> = ({ type, label, icon, className = '' }) => {
  ...
  return (
    <span
      data-status-badge
      className={`inline-flex items-center gap-[0.6em] rounded-[0.8em] border px-[1em] py-[0.4em] text-[10px] font-black uppercase tracking-wider transition-[color,background-color,border-color,box-shadow] duration-200 ${currentStyle.container} ${className}`}
    >
      {icon ??
