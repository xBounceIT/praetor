...
const ExternalListingView: React.FC<ExternalListingViewProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.supplierId?.trim()) {
      newErrors.supplierId = t('common:validation.supplierRequired');
    }
  ...
  const activeSuppliers = React.useMemo(() => suppliers.filter((s) => !s.isDisabled), [suppliers]);

  const supplierOptions: Option[] = React.useMemo(() => {
    const options = activeSuppliers.map((s) => ({ id: s.id, name: s.name }));
    if (formData.supplierId && !options.some((o) => o.id === formData.supplierId)) {
      const selectedSupplier = suppliers.find((s) => s.id === formData.supplierId);
      options.push({
        id: formData.supplierId,
        name: selectedSupplier?.name || t('common:labels.supplier'),
      });
    }
    return options;
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-2xl font-black text-slate-800">
              {t('crm:externalProducts.title')}
            </h2>
            <p className="text-slate-500 text-sm">{t('crm:externalProducts.subtitle')}</p>
          </div>
          <button
