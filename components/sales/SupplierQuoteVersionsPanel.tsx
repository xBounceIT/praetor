      ...
      <div className="hidden 2xl:flex w-72 max-h-[90vh] flex-col flex-shrink-0 rounded-2xl bg-white shadow-2xl overflow-hidden animate-in fade-in slide-in-from-right duration-200">
        <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <h4 className="text-sm font-black text-slate-800 flex items-center gap-2">
            ...
            {t('supplierQuotes.versionHistory.title')}
          </h4>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!isLoading && !error && rows.length === 0 && (
            <div className="p-6 text-xs text-slate-400 text-center leading-relaxed">
              {t('supplierQuotes.versionHistory.empty')}
            </div>
          )}
          {!isLoading && !error && rows.length > 0 && (
                ...
                return (
                  <li key={row.id}>
                    ...
                    >
                      ...
                      >
                        {row.reason === 'restore'
                          ? t('supplierQuotes.versionHistory.reasonRestore')
                          : t('supplierQuotes.versionHistory.reasonUpdate')}
                      </span>
                    </button>
                  </li>
                );
        ...
        {selectedVersionId && (
          <div className="border-t border-slate-100 p-3 space-y-2 bg-slate-50/50">
            ...
            >
              ...
              {t('supplierQuotes.versionHistory.backToCurrent')}
            </button>
            <button
              type="button"
            ...
            >
              ...
              {t('supplierQuotes.versionHistory.restoreButton')}
            </button>
          </div>
        )}
      </div>
