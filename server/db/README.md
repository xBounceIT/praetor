...
creation time, and id; it keeps one deterministic active row and copies every removed row in
full to `audit_logs` with action `time_entry.migration_duplicate_archived`. Deletion is guarded
by the matching archive record, so an archive collision or failure aborts the index build
