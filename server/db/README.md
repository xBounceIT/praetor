...
## Hourly-cost calendar rollout (migration 0112)

Deploy migration `0112_add_user_hourly_cost_periods.sql` before the application image. It is additive: it keeps `users.cost_per_hour`, creates one baseline period per existing user, and deliberately does not rewrite `time_entries` during migration. A compatibility trigger creates a baseline for inserts made by an older image and turns legacy scalar cost updates into a single global period. The new application continues to maintain `users.cost_per_hour` as today's compatibility value while date-based reads use the calendar.
