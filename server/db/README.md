...
`migrations/0000_baseline_pre_drizzle.sql` brings the already-modeled `notifications` table under drizzle's `__drizzle_migrations` tracking. It's hand-edited from drizzle-kit's `generate` output to use `CREATE TABLE IF NOT EXISTS` and `CREATE INDEX IF NOT EXISTS`, so it's a no-op on dev/prod DBs that already have the table from `schema.sql` while still bootstrapping a fresh DB cleanly. The snapshot in `meta/0000_snapshot.json` is intentionally left as drizzle-kit emitted it so future `db:generate` runs diff cleanly.

**Do not edit this file after merge.** drizzle-kit hashes each migration when it's applied; editing the SQL afterwards (even comments) causes a hash mismatch on any DB where it's already been applied. The same rule applies to every migration once it's committed.
