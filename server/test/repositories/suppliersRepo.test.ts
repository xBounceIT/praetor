...
describe('updateIfCodeAvailable', () => {
  test('updates without locking when supplierCode is omitted or cleared', async () => {
    exec.enqueue({ rows: [makeRow(SUPPLIER_ROW)] });
    ...
    exec.enqueue({ rows: [makeRow(SUPPLIER_ROW)] });
    expect(
      await suppliersRepo.updateIfCodeAvailable('s-1', { supplierCode: null }, testDb),
    ).toEqual({ ok: true, supplier: mappedRow });
    expect(exec.calls[1].sql.toLowerCase()).toContain('update "suppliers"');
    expect(exec.calls[1].sql).not.toContain('pg_advisory_xact_lock');
    expect(exec.calls[1].params).toContain(null);
  });

  test('locks and excludes the current supplier when changing code', async () => {
    exec.enqueue({ rows: [] });
    exec.enqueue({ rows: [] });
    exec.enqueue({ rows: [makeRow(SUPPLIER_ROW)] });
  ...
  test('returns duplicate_code when another supplier already owns the code', async () => {
    exec.enqueue({ rows: [] });
    exec.enqueue({ rows: [['ACM']] });
