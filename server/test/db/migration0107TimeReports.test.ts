...
const MIGRATION = readMigrationFile('0107_add_time_reports.sql');
const readJournal = async () =>
  Bun.file(new URL('../../db/migrations/meta/_journal.json', import.meta.url)).json();

describe('migration 0107: time reports', () => {
  ...
  test('is registered after migration 0106', async () => {
    const journal = (await readJournal()) as {
      entries: Array<{ idx: number; tag: string }>;
    };
    const index = journal.entries.findIndex((entry) => entry.tag === '0107_add_time_reports');
    ...
    expect(journal.entries[index]).toEqual(
      expect.objectContaining({ idx: 107, tag: '0107_add_time_reports' }),
    );
  });
});
