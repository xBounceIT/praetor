...
describe('POST /api/invoices', () => {
  ...
  test('201 retries autogenerated invoice id collision before returning success', async () => {
    generateNextIdMock.mockResolvedValueOnce('inv-1').mockResolvedValueOnce('inv-2');
    createMock
      .mockRejectedValueOnce(makeDbError('23505', 'invoices_pkey'))
      .mockResolvedValueOnce({ ...SAMPLE_INVOICE, id: 'inv-2' });
    insertItemsMock.mockResolvedValue([{ ...SAMPLE_ITEM, invoiceId: 'inv-2' }]);
    ...
    expect(res.statusCode).toBe(201);
    expect(JSON.parse(res.body).id).toBe('inv-2');
    expect(generateNextIdMock).toHaveBeenCalledTimes(2);
  ...
  test('409 autogenerated invoice id collisions stop after max retry attempts', async () => {
    generateNextIdMock.mockResolvedValue('inv-1');
    ...
    expect(generateNextIdMock).toHaveBeenCalledTimes(5);
    expect(withDbTransactionMock).toHaveBeenCalledTimes(5);
  ...
  test('409 caller-supplied invoice id collision is not retried', async () => {
    ...
    expect(res.statusCode).toBe(409);
    expect(JSON.parse(res.body)).toEqual({ error: 'Invoice ID already exists' });
    expect(generateNextIdMock).not.toHaveBeenCalled();
